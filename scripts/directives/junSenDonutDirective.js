(function() {
  myApp.directive("junSenDonutChart", junSenDonutDirective);
  junSenDonutDirective.$inject = ["d3Service", "$rootScope"];

  function junSenDonutDirective(d3Service, $rootScope) {
    return {
      restrict: 'EA',
      scope: {
        'config': '='
      ...
      link: function(scope, element, attrs, filter) {
        function setupTennisDonut() {
          ...
          //console.log("tennis parts curr" + scope.config.stateCurr.junSenDonutParts);
          var arcs = g.selectAll('path')
            .data(pie(scope.config.stateCurr.junSenDonutParts))
            .enter().append('path')
          ...
          var labels = g.selectAll('.label')
            .data(pie(scope.config.junSenNamesCurr))
            .enter().append("text")
            .attr("dy", ".35em")
            .attr("class", "label")
            ...
            .text(function(d) {
              ...
              //    console.log(scope.config.junSenNamesCurr);
              return d.data;
            });
          //legend
          ...
          scope.$watch(
            ...
            function(newVal, oldVal) {
              ...
              //   console.log(scope.config.stateCurr.junSenDonutParts);
              //   console.log("tennis donut current percentage:");
              var duration = 50;
              arcs.data(pie(scope.config.stateCurr.junSenDonutParts)); //.attr('d', arc);
              arcs.transition()
                .duration(duration)
